use brewdrivers::{model::RTU, drivers::InstrumentError, controllers::Controller, state::BinaryState};
...
async fn test_generate_and_update_device_state() -> Result<(), InstrumentError> {
    ...
    let res = RTU::generate(Some(brewdrivers::TEST_CONFIG_FILE));
    assert!(res.is_ok());
    let mut rtu = res.unwrap();
    assert!(rtu.devices.len() > 0);


    let relay = rtu.devices.iter_mut().find(|dev| *dev.conn.controller() == Controller::WaveshareV2 );
    ...
    let pid = rtu.devices.iter_mut().find(|dev| *dev.conn.controller() == Controller::CN7500 );
...
async fn test_device_enact() -> Result<(), InstrumentError> {
    ...
    let relay_again = rtu.devices.iter_mut().find(|dev| *dev.conn.controller() == Controller::WaveshareV2 );
